    ...
    return(
        <div  className="w-full h-full group cursor-pointer overflow-hidden relative">
            ...
            <div className="right-2 top-2 absolute w-fit flex items-end flex-col">
                ...
                <button onClick={() => setDropDownOpen(!dropDownOpen)} className="group-hover:opacity-100 hover:bg-white/50 xl:opacity-0  bg-white/70 w-7 rounded-[10px] px-2 duration-200 cursor-pointer text-center">...</button>
            ...
            <div onClick={onClick} className="w-full h-[43px] bg-white rounded-b-2xl group-hover:bg-white/80 duration-200 px-3 flex items-center group-hover:px-4">{emoji} {title}</div>
        </div>
    )
}